...
const WhereToGo = React.memo(({
  ...
  const headerText = useMemo(() => {
    if (homeOrWorkPlacetype) {
      return `${isEditHomePlaces ? "Edit" : ""} ${homeOrWorkPlacetype?.toUpperCase()} Place`;
    }
    ...
    <>
      <View style={[styles.container, { height }]}>
        <View style={styles.pickDropBtnCard}>
          <Text style={{ fontSize: 20, fontWeight: "bold" }}>{headerText}</Text>
          <LocationInput
            title={title}
          ...
          <View style={styles.mapFavoriteCard}>
            <TouchableOpacity onPress={() => setIsSendOrReceiveParcel("seggested")}>
              <Text
                style={{
                  fontSize: 12,
                  color: isSelectFavoritePlaces === "seggested" ? "#000" : "blue",
                }}
              >
        ...
        <LocationList
          data={isSelectFavoritePlaces === "favorite" ? favoritePlaces : suggestions ?? nearPlaces}
